...
### 8. Рефакторинг Telegram-транспорта (Файлы и Голосовые)
**Файл:** `extensions/telegram/src/channel.ts`
**Задачи:** Добавить скачивание `voice` и `document` во временную папку и пробрасывать агенту локальные пути. Реализовать склейку (Debouncing) пересланных подряд сообщений. Выпилить авто-ответы заглушки.
