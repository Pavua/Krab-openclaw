...
1. Реализован reusable cleanup script в `scripts/cleanup_old_inbox_items.py`
2. По умолчанию архивирует только stale `owner_request/open` старше 3 суток
3. Использует `InboxService.filter_by_age` + `InboxService.bulk_update_status`
4. Пишет actor `system-cleanup` и resolution note в workflow events
5. Живые stale items `10897` и `10848` закрыты в runtime state 2026-03-27

### Task 6: Checkpoint - Verify scheduler and cleanup improvements ✅
1. `pytest -q tests/unit/test_cleanup_old_inbox_items.py tests/unit/test_inbox_service.py -q` -> `23 passed`
2. `./venv/bin/python scripts/cleanup_old_inbox_items.py --dry-run` показал ровно 2 stale owner_request
3. `./venv/bin/python scripts/cleanup_old_inbox_items.py` успешно закрыл оба item-а
4. `curl http://127.0.0.1:8080/api/health/lite` после TTL-кеша показывает уже `pending_owner_requests=2`
