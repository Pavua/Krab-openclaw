...
LEGACY_BASELINE_PROJECT_READINESS = "~31%"
...
def _resolve_project_readiness(*, runtime_snapshot: dict[str, Any], ops_evidence: dict[str, Any]) -> str:
    ...
    workflow_summary = workflow_summary if isinstance(workflow_summary, dict) else {}
    pre_release = ((ops_evidence.get("pre_release_smoke_latest") or {}).get("summary")) or {}
    pre_release = pre_release if isinstance(pre_release, dict) else {}
    ...
    return LEGACY_BASELINE_PROJECT_READINESS
...
def _build_attach_summary_md(
    *,
    ...
    lines = [
        ...
        "- merge в `main` делать только дисциплинированным срезом проверенной ветки, а не всем локальным dirty worktree",
...
def main() -> int:
    ...
    project_readiness = _resolve_project_readiness(
    ...
    runtime_snapshot["project_readiness"] = project_readiness
    ...
    (BUNDLE_DIR / "PABLITO_RETURN_CHECKLIST.md").write_text(
        _build_pablito_return_checklist_md(
            runtime_snapshot=runtime_snapshot,
            project_readiness=project_readiness,
        ),
        encoding="utf-8",
    ...
    (BUNDLE_DIR / "THIRD_ACCOUNT_BOOTSTRAP_RU.md").write_text(
        _build_third_account_bootstrap_md(
            runtime_snapshot=runtime_snapshot,
            project_readiness=project_readiness,
        ),
        encoding="utf-8",
