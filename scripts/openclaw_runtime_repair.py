...
5. Retention cleanup session backup files (Wave 18-A)
...
def main() -> int:
    ...
    # Шаг 5: retention cleanup session backup files (Wave 18-A)
    # Запускается каждый раз при вызове repair (via /api/runtime/recover) —
    # не каждый boot, поэтому overhead приемлем. Non-fatal: ошибки здесь
    # не попадают в errors[], только в warnings[].
