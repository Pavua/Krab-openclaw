...
PROVIDER_SHAPE_FALLBACKS: dict[str, tuple[str, ...]] = {
...
def _seed_provider_shape(
    *,
    ...
    """
    Досеивает обязательные provider-level поля из близкого runtime-аналога.

    Почему это нужно:
    - некоторые CLI/OAuth-провайдеры живут на runtime alias-имени
      (`codex-cli`, `google-gemini-cli`), но полноценный provider catalog
      в live JSON может храниться только под sibling-провайдером;
    - простое добавление `models[]` без `baseUrl` даёт invalid config при reload.
    """
    changed_fields: list[str] = []
    for sibling_name in PROVIDER_SHAPE_FALLBACKS.get(str(provider or "").strip().lower(), ()):
        sibling_cfg = providers.get(sibling_name)
        if not isinstance(sibling_cfg, dict):
            continue

        base_url = str(sibling_cfg.get("baseUrl", "") or "").strip()
        if base_url and not str(provider_cfg.get("baseUrl", "") or "").strip():
            provider_cfg["baseUrl"] = base_url
            changed_fields.append("baseUrl")

        api_name = str(sibling_cfg.get("api", "") or "").strip()
        if api_name and not str(provider_cfg.get("api", "") or "").strip():
            provider_cfg["api"] = api_name
            changed_fields.append("api")

        # Для codex-cli auth truth живёт в CLI login-state, а не в auth-profiles.json.
        # Поэтому auth-поле копируем только для тех alias-провайдеров, где это безопасно.
        auth_name = str(sibling_cfg.get("auth", "") or "").strip()
        if (
            auth_name
            and str(provider or "").strip().lower() != "codex-cli"
            and not str(provider_cfg.get("auth", "") or "").strip()
        ):
            provider_cfg["auth"] = auth_name
            changed_fields.append("auth")
        break
...
def ensure_model_in_provider_catalog(
    ...
    *,
    ...
    seeded_fields = _seed_provider_shape(
        providers=providers,
        provider=provider,
    ...
    for item in models:
        if isinstance(item, dict) and str(item.get("id") or "").strip().lower() == target_lower:
            ...
            if bool(item.get("reasoning", False)) != bool(reasoning):
                ...
                changed = True
                reason_parts.append("reasoning_updated")
            if seeded_fields:
                changed = True
                reason_parts.append("provider_shape_seeded")
            return {
                "changed": changed,
                "reason": "already_present" if not changed else "+".join(reason_parts),
                "models_count": len(models),
                "provider_fields_seeded": seeded_fields,
            }
