...
def _merge_cli_args(existing_args: Any, preferred_args: list[str]) -> list[str]:
    """
    Нормализует аргументы Codex CLI под fully-unrestricted runtime.

    Почему это делаем именно здесь:
    - после обновлений OpenClaw `cliBackends.codex-cli.args` может частично
      сохраняться из старого safe-профиля;
    - пользователь запускает Краба через launcher, значит bootstrap — самое
      устойчивое место, где можно каждый раз возвращать нужный execution mode;
    - при этом мы сохраняем редкие кастомные аргументы, если они не конфликтуют
      с отключением sandbox/approvals.
    """
    normalized_existing = [
        str(item).strip()
        for item in (existing_args if isinstance(existing_args, list) else [])
        if str(item).strip()
    ]
    blocked_flags = {
        "--full-auto",
    ...
    for item in normalized_existing:
        if skip_next:
            skip_next = False
            continue
        if item in blocked_flags:
            # Старые sandbox-флаги не должны переопределять break-glass режим.
            skip_next = item == "--sandbox"
            continue
        if item == "--dangerously-bypass-approvals-and-sandbox":
            continue
        filtered_existing.append(item)

    merged = list(preferred_args)
...
def _ensure_codex_cli_backend(defaults: dict[str, Any]) -> None:
    """Гарантирует, что Codex backend стартует без sandbox/approvals."""
    cli_backends = _ensure_dict(defaults, "cliBackends")
    codex_backend = _ensure_dict(cli_backends, "codex-cli")

    command = str(codex_backend.get("command") or "").strip()
    if not command:
        codex_backend["command"] = DEFAULT_CODEX_CLI_COMMAND

    codex_backend["args"] = _merge_cli_args(codex_backend.get("args"), DEFAULT_CODEX_CLI_ARGS)
    codex_backend.setdefault("output", "jsonl")
    codex_backend["resumeArgs"] = _merge_cli_args(
        codex_backend.get("resumeArgs"),
        DEFAULT_CODEX_CLI_RESUME_ARGS,
    )
    codex_backend.setdefault("resumeOutput", "jsonl")
...
def _seed_runtime_config(config_path: Path, models_path: Path) -> dict[str, Any]:
    ...
    _ensure_codex_cli_backend(defaults)
