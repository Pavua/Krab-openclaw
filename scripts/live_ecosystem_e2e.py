...
async def _voice_lifecycle(client: VoiceGatewayClient) -> dict[str, Any]:
    ...
    if missing:
        result["skipped"] = True
        result["reason"] = f"voice_client_missing_methods:{','.join(missing)}"
        return result
...
async def main() -> int:
    parser = argparse.ArgumentParser(description="Live E2E проверка экосистемы Krab")
    parser.add_argument(
        "--require-openclaw",
        action="store_true",
        help="Считать проверку failed, если OpenClaw gateway недоступен",
    )
    parser.add_argument(
        "--require-ear",
        action="store_true",
        help="Считать проверку failed, если Krab Ear (IPC/HTTP) недоступен",
    )
    parser.add_argument(
        "--require-voice-lifecycle",
        action="store_true",
        help="Считать проверку failed, если voice lifecycle не выполнен/неуспешен",
    )
    args = parser.parse_args()

    openclaw_base = (
        os.getenv("OPENCLAW_BASE_URL", "")
        or os.getenv("OPENCLAW_URL", "http://127.0.0.1:18789")
    ).strip().rstrip("/")
    ...
    krab_ear_base = os.getenv("KRAB_EAR_BACKEND_URL", "http://127.0.0.1:5005").strip().rstrip("/")
    ...
    # Клиентские checks (с учетом IPC/Fallback логики интеграционных клиентов).
    voice_client = VoiceGatewayClient(base_url=voice_base, api_key=voice_api_key or None)
    ear_client = KrabEarClient(base_url=krab_ear_base)
    checks_client = {
        "voice_gateway": await voice_client.health_report(),
        "krab_ear": await ear_client.health_report(),
    }
    ...
    if checks_http["voice_gateway"]["ok"]:
        lifecycle = await _voice_lifecycle(voice_client)
        if "skipped" not in lifecycle:
            lifecycle["skipped"] = False
            lifecycle["reason"] = ""

    # Деградация AI цепочки: cloud -> local fallback.
    cloud_ok = bool(checks_http["openclaw"]["ok"])
    local_ok = bool(checks_http["local_lm"]["ok"])
    if cloud_ok:
        degradation = "normal"
    elif local_ok:
    ...
    voice_ok = bool(checks_http["voice_gateway"]["ok"])
    lifecycle_ok = bool(lifecycle.get("ok"))
    lifecycle_required = voice_ok and (not bool(lifecycle.get("skipped")))
    overall_ok = ai_backend_ok and ((not lifecycle_required) or lifecycle_ok)
    if args.require_openclaw and (not checks_http["openclaw"]["ok"]):
        overall_ok = False
    if args.require_ear and (not checks_client["krab_ear"]["ok"]):
        overall_ok = False
    if args.require_voice_lifecycle and (not lifecycle_ok):
        overall_ok = False
    ...
    payload = {
        ...
        "voice_lifecycle_required": lifecycle_required,
        "strict_flags": {
            "require_openclaw": bool(args.require_openclaw),
            "require_ear": bool(args.require_ear),
            "require_voice_lifecycle": bool(args.require_voice_lifecycle),
        },
    }
