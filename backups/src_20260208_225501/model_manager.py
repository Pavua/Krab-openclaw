...
class ModelManager:
    ...
    async def select_best_model(self, task_type: str = "chat") -> str:
        """
        Выбирает и загружает лучшую модель
        """
        defaults = {
        ...
        # 1. Сначала ищем в доступных локальных
        available = await self.discover_models()
        best_local = None
        
        target = defaults.get(task_type, "gemini")
        if target == "gemini":
            return config.GEMINI_API_KEY and "google/gemini-2.0-flash-exp" or "local"
