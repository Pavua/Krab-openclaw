...
- Причина: после предыдущих фиксов recovery уже работал, но обычный reload всё ещё показывал голые `—` в части owner-карточек до завершения сетевых вызовов, хотя у вкладки уже был подтверждённый last-good state. Дополнительно transient fetch-failure мог поверх уже поднятого cache снова записать `ERR/FAIL`.
- Что сделано: в [src/web/index.html](/Users/pablito/Antigravity_AGENTS/Краб/src/web/index.html) расширен `localStorage`-bootstrap cache `krab:owner-panel-bootstrap:v1`; туда сохраняются last-good snapshot'ы секций `OpenClaw`, `Voice Runtime`, `Translator`, `Cron`, `Local Lifecycle`, `Compat`, `Effective Routing` и верхнего `dashboardStats` (включая safe HTML-контейнеры вроде `checks/services`). На старте страницы вызывается `applyOwnerPanelBootstrapCache()` до `refreshAll()`, поэтому high-value runtime-блоки поднимаются из truthful cache сразу, а volatile `Browser / MCP` остаётся в честном `LOADING`. Error-path для `catalog/autoswitch/channels/localLifecycle` теперь не затирает already-applied cache в `ERR/FAIL`, если live fetch временно недоступен. Добавлена регрессия в [tests/unit/test_web_panel_bootstrap_order.py](/Users/pablito/Antigravity_AGENTS/Краб/tests/unit/test_web_panel_bootstrap_order.py).
- Проверка:
  ...
  - cache-only acceptance с полностью оборванными `/api/*`: первый кадр всё равно показывает `degradation = stable`, `Black Box = 12`, `RAG = 34`, `Local Model = LM Studio idle`, `Route mode = auto`, `Autoswitch = OK`, `Voice delivery = text+voice`, `Local Lifecycle = WARN`
  ...
  - артефакты: [owner-panel-cached-first-paint-20260328-1514.png](/Users/pablito/Antigravity_AGENTS/Краб/output/playwright/owner-panel-cached-first-paint-20260328-1514.png), [owner-panel-cache-first-paint-20260328-1619.png](/Users/pablito/Antigravity_AGENTS/Краб/output/playwright/owner-panel-cache-first-paint-20260328-1619.png)
