...
async def test_cloud_failover_chain_smoke(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        client,
        "_switch_cloud_tier",
        AsyncMock(return_value={"ok": False, "error": "invalid_paid_key_type"}),
    )

    from src.model_manager import model_manager

    monkeypatch.setattr(
        model_manager, "get_best_model", AsyncMock(return_value="google/gemini-2.5-flash")
    )
    ...
    monkeypatch.setattr(
        model_manager,
        "resolve_preferred_local_model",
        AsyncMock(return_value="local/zai-org/glm-4.6v-flash"),
    )
    monkeypatch.setattr(
        client,
    ...
    assert client.get_tier_state_export()["last_error_code"] in {
        "model_not_loaded",
