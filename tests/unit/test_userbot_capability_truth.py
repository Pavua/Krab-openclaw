...
def test_looks_like_commands_question_detects_help_intent() -> None:
    """Fast-path отключён по просьбе пользователя — функция всегда возвращает False."""
    # NOTE: отключено ("все вопросы уходят в LLM")
    assert KraabUserbot._looks_like_commands_question("Какие у тебя команды?") is False
    assert KraabUserbot._looks_like_commands_question("Покажи список команд") is False
...
def test_looks_like_integrations_question_detects_runtime_tools() -> None:
    """Fast-path отключён по просьбе пользователя — функция всегда возвращает False."""
    # NOTE: отключено ("все вопросы уходят в LLM")
    assert KraabUserbot._looks_like_integrations_question("Какие у тебя интеграции?") is False
    assert KraabUserbot._looks_like_integrations_question("Что у тебя подключено?") is False
...
@pytest.mark.skip(reason="commands fast-path disabled per user request — все вопросы уходят в LLM")
@pytest.mark.asyncio
