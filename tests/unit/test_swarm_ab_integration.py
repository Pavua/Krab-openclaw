...
async def test_round_with_active_ab_control_uses_baseline(tmp_path):
    ...
    mock_curator = MagicMock()
    mock_curator.get_active_ab_test.return_value = "analysts-20260504-120000"
    mock_curator.select_variant.return_value = "control"
    mock_curator.get_ab_test.return_value = {
...
async def test_round_with_active_ab_candidate_uses_candidate_prompt(tmp_path):
    ...
    mock_curator = MagicMock()
    mock_curator.get_active_ab_test.return_value = "coders-20260504-130000"
...
async def test_record_round_metric_called_after_completion(tmp_path):
    ...
    mock_curator = MagicMock()
    mock_curator.get_active_ab_test.return_value = "traders-20260504-140000"
    mock_curator.select_variant.return_value = "candidate"
    mock_curator.get_ab_test.return_value = {
...
def test_start_and_get_active_ab_test(tmp_path):
    ...
    # После start — возвращает ab_id
    with patch("src.core.skill_curator.CURATOR_STATE_PATH", tmp_path / "state.json"):
        ab_id = curator.start_ab_test("analysts", "Candidate prompt text")
        assert ab_id.startswith("analysts")

        # get_active_ab_test должен найти его
        found = curator.get_active_ab_test("analysts")
        assert found == ab_id

        # Второй start должен вызвать ValueError
        with pytest.raises(ValueError, match="A/B-тест уже активен"):
            curator.start_ab_test("analysts", "Another candidate")
