...
async def test_is_lm_studio_available_returns_true_on_200():
    with patch("src.core.local_health.build_lm_studio_auth_headers", return_value={"Authorization": "Bearer lm-token"}):
    ...
    assert mock_httpx.AsyncClient.call_args.kwargs["headers"] == {"Authorization": "Bearer lm-token"}
