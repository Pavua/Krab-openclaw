...
| Session 27 | **10197 passed**, 94 skipped, 3 pre-existing fails (auth_recovery / policy_matrix / inbox_status — НЕ от Phase 2 split), 7 infra-hangers исключены. **Phase 2 command_handlers split COMPLETE** — 19637 → 11041 LOC (−43.8%) через 10 waves: text_utils / chat / scheduler / voice / memory / social / ai / swarm / translator / system. +Test-fix dual-namespace lookup pattern (158 регрессий → 0, fbf3262). +Smart Routing analyzer (`scripts/analyze_smart_routing.py` + 22 tests, 0c7f89d). +5-layer LLM recovery 27.04 (env defenses + harness `codex-cli` → `codex` rename + Codex CLI 0.115 → 0.125 + stale defaults purge 68111fc + OAuth refresh) |
