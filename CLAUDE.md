...
| Session 27 | **10431 passed**, 94 skipped, **0 failed** (1 hanger вне scope `test_web_model_api.py` исключён; pytest-timeout=30 default установлен). Phase 2 command_handlers split: 19637 → 9333 LOC (**−52.5%**) через 11 waves: text_utils / chat / scheduler / voice / memory / social / ai / swarm / translator / system / **admin** (Wave 11 — 22 handlers). +Test-fix dual-namespace lookup pattern (158→0 раунд 1 в fbf3262, +49→0 раунд 2 в 847786f). +Smart Routing analyzer (`scripts/analyze_smart_routing.py` + 22 tests, 0c7f89d). +5-layer LLM recovery 27.04 (env defenses + harness `codex-cli` → `codex` rename + Codex CLI 0.115 → 0.125 + stale defaults purge 68111fc + OAuth refresh). +pytest-timeout 2.4.0 + 7 hangers RCA (`subprocess.run` в `build_auth_recovery_readiness_snapshot` blocking asyncio, mocked в 5 файлах, 3ca34a0). +3 pre-existing fails repaired (auth_recovery / policy_matrix / inbox_status, d4fb33e). +Yung Nagato persona update (userbot vs reserve_bot transport channels, baa00e6) |
