...
const DEFAULT_EXTERNAL_INSTRUCTION = [
  ...
  "4) Если функция доступна только в отдельном Python userbot-контуре, прямо так и говори: 'это доступно в userbot, но не подтверждено для этого внешнего канала'.",
  ...
  "6) Для внешних каналов приоритет: текстовый truthful-ответ лучше, чем выдуманный self-check или неподтверждённое обещание.",
  "7) Если не уверен в доступности интернета, браузера или автодействий, явно скажи, что доступ не подтверждён.",
].join("\n");
...
function applyExternalTruthGuard(text) {
  ...
  if (/фактический runtime self-check/iu.test(out)) {
    return "Связь в этом канале есть. Reserve Telegram Bot не подтверждает полный runtime self-check; основной owner-канал живёт в Python userbot.";
  }

  if (/(?:я сейчас работаю на модели|сейчас я работаю на модели)\s+`?[^\n`]+`?/iu.test(out)) {
    const configuredPrimaryMatch = out.match(
      /дефолтн(?:ая|ой)\s+модель(?:[\s\S]{0,120}?настроена как)?\s+`?([^\n`)]+)`?(?:\)|\n|$)/iu,
    );
    const primary = String(configuredPrimaryMatch?.[1] || "").trim().replace(/[.)\s]+$/g, "");
    if (primary) {
      return `В этом диалоге отвечает reserve Telegram Bot. Primary в runtime настроен как \`${primary}\`; фактический активный маршрут здесь нужно подтверждать по runtime-route, а не по свободному тексту модели.`;
    }
    return "Маршрут ответа в этом канале нужно подтверждать по фактическому runtime-route, а не по шаблонному тексту модели.";
  }
  ...
  const replacements = [
    ...
    {
      pattern:
        /(?:нет,?\s*по\.?\s*)?я сейчас работаю через обычного telegram-бота(?:[\s\S]{0,180})?python userbot-контур(?:[\s\S]{0,120})?(?:не пересаживали|не переводили)\.?/giu,
      value: "В этом диалоге отвечает reserve Telegram Bot. Основной owner-канал живёт в Python userbot; память общая, но owner-инструменты здесь не подтверждены.",
    },
  ];
