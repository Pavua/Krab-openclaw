...
from ..core.access_control import (
    PARTIAL_ACCESS_COMMANDS,  # noqa: F401 — patch surface (admin_commands re-uses via _ch)
    AccessLevel,
    get_effective_owner_label,  # noqa: F401 — patch surface
    load_acl_runtime_state,  # noqa: F401 — patch surface
    normalize_subject,  # noqa: F401 — patch surface
    update_acl_subject,  # noqa: F401 — patch surface
)
from ..core.chat_ban_cache import chat_ban_cache  # noqa: F401 — patch surface
...
from ..employee_templates import (  # ROLES noqa: F401 — patch surface
    get_role_prompt,
    list_roles,
    save_role,
)
...
# ---------------------------------------------------------------------------
# Административные / конфигурационные handlers (!config, !set, !acl, !scope,
# !reasoning, !role, !notify, !chatban, !cap, !silence, !archive, !unarchive)
# и их private helpers вынесены в commands/admin_commands.py.
# Re-exported для обратной совместимости (тесты, test_config_command.py).
# ВАЖНО: _AUTODEL_STATE_KEY остаётся в этом модуле (источник истины).
from .commands.admin_commands import (  # noqa: E402, F401
    _CONFIG_GROUPS,  # noqa: F401 — patch surface для тестов
    _CONFIG_KEY_DESC,  # noqa: F401 — patch surface
    _SET_ALIASES,  # noqa: F401
    _SET_FRIENDLY,  # noqa: F401
    _get_set_value,  # noqa: F401
    _render_all_settings,  # noqa: F401
    _render_chat_ban_entries,  # noqa: F401
    _render_config_all,  # noqa: F401
    _render_config_value,  # noqa: F401
    handle_acl,
    handle_archive,
    handle_cap,
    handle_chatban,
...
# Извлечено в commands/admin_commands.py (Phase 2 Wave 11). Re-exported ниже.
# Локальные определения удалены — используй import через admin_commands.
##############################################################################

# (config_key, описание)
# _CONFIG_GROUPS — extracted to admin_commands.py (Phase 2 Wave 11). Re-exported above.

# Плоский индекс key→описание для быстрого поиска
# _CONFIG_KEY_DESC — extracted to admin_commands.py (Phase 2 Wave 11). Re-exported above.

# _render_config_value — extracted to admin_commands.py (Phase 2 Wave 11). Re-exported above.
# _render_config_all — extracted to admin_commands.py (Phase 2 Wave 11). Re-exported above.
##############################################################################
# Алиасы !set: короткое имя → реальный ключ config (или специальный режим)
##############################################################################
# _SET_ALIASES — extracted to admin_commands.py (Phase 2 Wave 11). Re-exported above.

# (алиас → (config_key_или_метка, описание))
# _SET_FRIENDLY — extracted to admin_commands.py (Phase 2 Wave 11). Re-exported above.


# _get_set_value — extracted to admin_commands.py (Phase 2 Wave 11). Re-exported above.
# _render_all_settings — extracted to admin_commands.py (Phase 2 Wave 11). Re-exported above.
...
# _render_chat_ban_entries — extracted to admin_commands.py (Phase 2 Wave 11). Re-exported above.
...
# handle_cap — extracted to commands/admin_commands.py (Phase 2 Wave 11). Re-exported above.
...
# handle_silence — extracted to commands/admin_commands.py (Phase 2 Wave 11). Re-exported above.
...
# handle_archive — extracted to commands/admin_commands.py (Phase 2 Wave 11). Re-exported above.
# handle_unarchive — extracted to commands/admin_commands.py (Phase 2 Wave 11). Re-exported above.
