...
class EventTimeline:
    ...
    def get_events(
        self, limit: int = 200, min_severity: Optional[str] = None, channel: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        result = []
...
def mask_secrets(payload: Any) -> Any:
    ...
    if isinstance(payload, dict):
        ...
        for k, v in payload.items():
            k_lower = str(k).lower()
            if any(
                s in k_lower for s in ("api_key", "secret", "token", "password", "authorization")
            ):
                masked[k] = "***MASKED***"
            else:
...
def build_ops_response(
    status: str, error_code: str = "", summary: str = "", data: Optional[Dict[str, Any]] = None
) -> Dict[str, Any]:
    """
    Standardized operational response format.
    status: ok | degraded | failed
    """
    return {"status": status, "error_code": error_code, "summary": summary, "data": data or {}}
