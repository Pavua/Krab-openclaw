...
_SCORE_WEIGHT_TOOL_CALLS = 1.0  # бонус за рост tool_calls
...
def _mock_swarm_round(team: str, prompt: str, round_id: str) -> dict[str, Any]:
    """Детерминированный stub раунда для dry-run / тестов (никаких LLM вызовов)."""
    h = abs(hash(f"{team}:{prompt[:50]}:{round_id}")) % 100
    return {
        "round_id": round_id,
        "error": False,
        "ttft_s": 0.5 + (h % 10) * 0.05,
        "response_quality": 0.6 + (h % 4) * 0.1,
        "tool_calls": h % 3,
    }
...
class SkillCuratorABRunner:
    ...
    Запускает N сравнительных раундов (dry-run/mock режим по умолчанию).
    Реальный swarm-invoke можно подключить через инъекцию ``swarm_invoker``.
    """
        ...
        *,
        ...
        for i in range(n):
            baseline_rounds.append(
                await self._run_single_round(team, baseline_prompt, f"baseline-{i}")
            )
        for i in range(n):
            candidate_rounds.append(
                await self._run_single_round(team, candidate_prompt, f"candidate-{i}")
            )
        ...
        logger.info("curator_ab_runner_done", team=team, rounds=n, delta_score=delta_score)
        return result

    def _load_baseline_prompt(self, team: str) -> str:
        """Читает текущий system prompt команды (graceful fallback)."""
        try:
            from .swarm_team_prompts import get_team_system_prompt
    ...
    async def _run_single_round(self, team: str, prompt: str, round_id: str) -> dict[str, Any]:
        """Выполняет один раунд через invoker или mock."""
    ...
    def _aggregate_metrics(rounds: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        n = len(rounds)
        errors = sum(1 for r in rounds if r.get("error", False))
        ttfts = [float(r.get("ttft_s", 0.0)) for r in rounds]
        qualities = [float(r.get("response_quality", 0.0)) for r in rounds]
        tool_calls = [float(r.get("tool_calls", 0)) for r in rounds]
        return {
            "count": n,
            "error_rate": round(errors / n, 6),
            "ttft_s_avg": round(sum(ttfts) / n, 6),
            "response_quality_avg": round(sum(qualities) / n, 6),
            "tool_calls_avg": round(sum(tool_calls) / n, 6),
        }
...
def auto_apply_if_threshold(
    ...
    *,
    ...
    ``_pending_skill_improvements.json`` (НЕ пишем в live prompts).
    ...
    if ok:
        ...
        logger.info(
            "curator_auto_apply_queued", team=team, entry_id=entry_id, delta_score=delta_score
        )
