...
Env-переменные (исторический legacy-нейминг, более ранний Wave 29-RR):
...
Альтернативный нейминг (Wave 65-G, единый префикс KRAB_):
    KRAB_LMSTUDIO_AUTO_UNLOAD_AFTER_IDLE_SEC — alias к LM_STUDIO_IDLE_UNLOAD_SEC.

Прецеденс: новый KRAB_-флаг переопределяет legacy при одновременной установке.
...
def _get_threshold_sec() -> float:
    """
    Возвращает порог idle-unload (сек).

    Прецеденс:
      1) KRAB_LMSTUDIO_AUTO_UNLOAD_AFTER_IDLE_SEC (Wave 65-G nameing)
      2) LM_STUDIO_IDLE_UNLOAD_SEC (legacy Wave 29-RR)
      3) default 600.0
    """
    raw = (
        os.environ.get("KRAB_LMSTUDIO_AUTO_UNLOAD_AFTER_IDLE_SEC")
        or os.environ.get("LM_STUDIO_IDLE_UNLOAD_SEC")
        or "600"
    )
    try:
        return float(raw)
