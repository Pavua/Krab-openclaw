...
class RuntimeStatusMixin:
    ...
    def _build_command_access_denied_text(command_name: str, access_profile: AccessProfile) -> str:
        ...
        W32: текст НЕ содержит буквального «!command» — иначе другие боты группы
        (спам-фильтры, какие-то auto-responders) ловят подстроку своим regex'ом
        и отвечают «!команда» обратно → мы снова триггеримся → loop.
        Используем название без `!` префикса и в backtick-кавычках.
        """
        command = str(command_name or "").strip().lower()
        if access_profile.level == AccessLevel.PARTIAL:
            return (
                f"🔒 Эта служебная команда ({command}) недоступна в режиме частичного доступа.\n"
                "Доступно: статус, помощь, поиск.\n"
                "Для расширения прав владелец должен перевести контакт в full-доступ."
            )
        return (
            f"🔒 Эта служебная команда ({command}) доступна только доверенному контуру Краба.\n"
