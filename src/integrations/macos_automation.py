...
class MacOSAutomationService:
    ...
    async def _run_osascript(self, script: str, *argv: str, timeout_sec: float | None = None) -> str:
    ...
    async def list_notes(self, limit: int = 20) -> list[dict[str, str]]:
        ...
        for line in output.splitlines():
            ...
            if len(parts) == 3:
                ...
                rows.append({"account_name": account_name, "folder_name": folder_name, "title": note_title})
        return rows
    ...
    async def list_upcoming_calendar_events(
        ...
        for line in output.splitlines():
            ...
            if len(parts) == 3:
                ...
                rows.append({"calendar_name": calendar_name, "title": title, "start_label": start_label})
        return rows
    ...
    async def create_calendar_event(
        ...
        *,
        ...
        event_title = title
        argv: list[str] = [calendar_name, event_title, *self._datetime_argv(start_at), *self._datetime_argv(end_at)]
