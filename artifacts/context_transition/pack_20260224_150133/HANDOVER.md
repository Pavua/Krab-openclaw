...
## ✅ v8 Sprint Update 24 (2026-02-23) — R17 Stability + Anti-413 Recovery Button

### Что реализовано

1. **Anti-413 API в Web Panel (`src/modules/web_app.py`)**:
   - добавлены write-endpoint'ы:
     - `POST /api/context/checkpoint`
     - `POST /api/context/transition-pack`
   - добавлен read-endpoint:
     - `GET /api/context/latest`
   - write-endpoint'ы защищены текущей политикой `_assert_write_access` (`X-Krab-Web-Key`/`token`).
   - добавлен унифицированный запуск локальных `.command`-скриптов:
     - timeout,
     - нормализованные коды ошибок,
     - ограничение `stdout_tail` (2000 символов).

2. **Расширен `GET /api/links`**:
   - добавлены ссылки:
     - `context_checkpoint_api`
     - `context_transition_pack_api`
     - `context_latest_api`

3. **UI: блок “Anti-413 Recovery” (`src/web/index.html`)**:
   - добавлены кнопки:
     - `Create Checkpoint`
     - `Build Transition Pack`
     - `Refresh Context Links`
   - добавлены функции:
     - `runContextCheckpoint()`
     - `runContextTransitionPack()`
     - `loadContextLatest()`
     - `renderContextLinks(payload)`
   - в интерфейсе показываются актуальные пути к артефактам (`checkpoint`, `pack`, `TRANSFER_PROMPT_RU.md`, `FILES_TO_ATTACH.txt`).

4. **Queue Stability guard mapping (R17 hardening)**:
   - в UI-метриках `opsGuardTriggers` теперь берётся из:
     - `reply_incomplete_guard_triggered || guard_triggers || guard_blocks || 0`.

5. **Тесты (`tests/test_web_app.py`)**:
   - добавлены:
     - `test_context_checkpoint_endpoint_success`
     - `test_context_transition_pack_endpoint_success`
     - `test_context_write_endpoints_require_api_key`
     - `test_context_latest_endpoint_returns_paths`
     - `test_links_endpoint_contains_openclaw_cloud_api_and_context_urls`

6. **Документация**:
   - `docs/CHAT_TRANSITION_PLAYBOOK_RU.md` дополнен разделом:
     - `Через Web Panel (кнопкой)`.

7. **Krab Ear clean HTTP health (strict mode, P0 runtime fix)**:
   - дефолт `KRAB_EAR_BACKEND_URL` в Krab переключён на `http://127.0.0.1:5005`
     (фактический порт `Krab Ear` REST backend).
   - обновлены one-click скрипты с `launchd`-управлением REST:
     - `start_krab_ear_backend.command`:
       - сначала пытается поднять REST как `launchd`-сервис `ai.krab.ear.rest`;
       - если `launchd` недоступен, использует fallback `start_rest_service.command`;
       - ждёт `GET /health` и пишет out/err логи REST.
     - `stop_krab_ear_backend.command`:
       - останавливает native `KrabEarAgent`, `launchd`-сервис REST и остаточные процессы `rest_server.py`.
     - `status_krab_ear_backend.command`:
       - показывает статус агента, `launchd` REST, процесса REST и HTTP health.
   - `scripts/live_ecosystem_e2e.py` и `src/core/ecosystem_health.py` переведены в **strict HTTP** для Krab Ear:
     - удалён process fallback (`process_probe`);
     - `krab_ear.ok=true` только при реальном `HTTP 200` на `/health`.
   - добавлен regression-тест:
     - `tests/test_ecosystem_health.py::test_krab_ear_http_error_does_not_use_process_fallback`.
   - обновлена документация:
     - `.env.example` (добавлен `KRAB_EAR_BACKEND_URL=http://127.0.0.1:5005`);
     - `docs/E2E_THREE_PROJECTS.md` (актуальный default URL для Krab Ear).

8. **Krab Core start-resilience (anti-silent mode)**:
   - `krab_core_daemon_start.command`:
     - если `launchctl bootstrap` падает (`Input/output error`), включается fallback:
       обычный background-старт `python -m src.main` с логом `logs/krab_manual_bg.log`;
     - добавлена проверка `http://127.0.0.1:8080/api/health` после старта.
   - `krab_core_daemon_status.command`:
     - показывает не только launchd-состояние, но и fallback PID + HTTP health.
   - `krab_core_daemon_stop.command`:
     - останавливает и launchd-сервис, и fallback background-процесс.
   - `src/handlers/ai.py`:
     - в аварийном пути `auto_reply` добавлена нормализация runtime-ошибок,
       чтобы в Telegram не уходили сырые `400 ...`, `<|begin_of_box|>...` и другие тех-артефакты.

### Верификация (v8-24)

1. Targeted pytest:
   - `pytest -q tests/test_web_app.py` → ✅ `41 passed`.
   - `pytest -q tests/test_openclaw_client_health.py tests/test_r17_cloud_diagnostics.py tests/test_model_router_stream_fallback.py tests/test_r16_queue_sla_abort.py tests/test_queue_timeout_user_notification.py` → ✅ `55 passed`.

2. Browser smoke (Playwright CLI, реальный браузер):
   - открыт `http://127.0.0.1:8080`;
   - нажаты `Create Checkpoint` и `Build Transition Pack`;
   - подтверждено обновление статуса в блоке `Anti-413 Recovery`;
   - подтверждено создание артефактов:
     - `artifacts/context_checkpoints/checkpoint_20260223_141719.md`
     - `artifacts/context_transition/pack_20260223_141719/`
   - screenshot:
     - `output/playwright/anti413_recovery_smoke_20260223_1417.png`.

3. Финальный gate (дополнительно, после feature-check):
   - `python scripts/live_ecosystem_e2e.py` → ✅ `overall_ok=true`
     - `report_file=artifacts/ops/live_ecosystem_e2e_20260223_135358Z.json`
     - `checks.krab_ear.status_code=200`, `url=http://127.0.0.1:5005/health` (strict HTTP).
   - `python scripts/pre_release_smoke.py --full` → ✅ `required_failed=0`, `advisory_failed=0`
     - `report=artifacts/ops/pre_release_smoke_20260223_142732.json`.

4. Post-fix regression:
   - `pytest -q tests/test_ecosystem_health.py tests/test_web_app.py tests/test_openclaw_client_health.py tests/test_r17_cloud_diagnostics.py tests/test_model_router_stream_fallback.py tests/test_r16_queue_sla_abort.py tests/test_queue_timeout_user_notification.py` → ✅ `100 passed`.

5. Clean HTTP health verification (Krab Ear):
   - `./start_krab_ear_backend.command` → поднимает REST health и не дублирует agent, если он уже запущен.
   - `./status_krab_ear_backend.command` → ✅ `REST launchd: UP`, ✅ `REST процесс: UP`, ✅ `HTTP health: UP (http://127.0.0.1:5005/health)`.
   - контроль через паузу отдельной командой (`sleep 6 && ./status_krab_ear_backend.command`) сохраняет `HTTP health: UP`.
   - `python scripts/live_ecosystem_e2e.py` → ✅ `report_file=artifacts/ops/live_ecosystem_e2e_20260223_135358Z.json`
     с `checks.krab_ear.ok=true` только через HTTP (`status_code=200`).

6. Krab Core fallback verification (launchd error path):
   - `./krab_core_daemon_start.command` в окружении с `launchd bootstrap failed: 5`:
     - ✅ автоматически переключается на fallback background-режим;
     - ✅ поднимает `http://127.0.0.1:8080/api/health`.
   - `./krab_core_daemon_status.command`:
     - ✅ показывает fallback `PID` и `HTTP health: UP`.
   - `./krab_core_daemon_stop.command`:
     - ✅ останавливает fallback-процесс.
   - regression:
     - `pytest -q tests/test_ai_runtime_error_guard.py tests/test_model_router_output_sanitizer.py tests/test_model_router_stream_fallback.py`
       → ✅ `34 passed`.

---

## ✅ v8 Sprint Update 25 (2026-02-23) — OpenClaw Health Fallback + Key Reality Check

### Что реализовано

1. **Стабилизация `OpenClawClient.health_check` при HTML control UI fallback** (`src/core/openclaw_client.py`):
   - добавлен CLI-fallback через `openclaw channels status --probe`, если `/health` или `/v1/models` возвращают HTML вместо JSON API;
   - добавлен короткий TTL-cache результата fallback (`5 сек`);
   - добавлен env-флаг отключения fallback:
     - `OPENCLAW_HEALTH_CLI_FALLBACK=0` (для строгого режима/тестов).

2. **Тесты на новый health-путь** (`tests/test_openclaw_client_health.py`):
   - `HTML + CLI ok => health=True`;
   - `HTML + CLI fail => health=False`;
   - `HTML + fallback disabled => health=False`.

3. **Уточнение daemon-скриптов по PID-поиску**:
   - обновлён поиск Python-процесса ядра в:
     - `krab_core_daemon_start.command`
     - `krab_core_daemon_stop.command`
     - `krab_core_daemon_status.command`
   - используется `ps + awk` по `-m src.main` / `src/main.py` вместо нестабильного regex-шаблона `pgrep`;
   - добавлен case-insensitive матч по `python/Python` в команде процесса;
   - в `krab_core_daemon_status.command` добавлен retry health-probe (`/api/health`), чтобы снизить ложные `DOWN` из-за короткого таймаута.

### Диагностика ключей (фактическая проверка на 2026-02-23)

1. **Gemini FREE key** (`GEMINI_API_KEY_FREE`) — рабочий (ошибки Google API не вернул).
2. **Gemini PAID key** (`GEMINI_API_KEY_PAID`) — невалиден как API key для Gemini endpoint:
   - ответ: `Google API 401: API keys are not supported by this API...`
   - вывод: в paid-поле сейчас ключ другого типа/формата.
3. **OpenAI key** (`OPENAI_API_KEY`) — рабочий (прямой probe `GET /v1/models` без 401/403).

### Верификация

1. `pytest -q tests/test_openclaw_client_health.py` → ✅ `23 passed`.
2. `pytest -q tests/test_openclaw_client_health.py tests/test_r17_cloud_diagnostics.py tests/test_model_router_stream_fallback.py tests/test_r16_queue_sla_abort.py tests/test_queue_timeout_user_notification.py` → ✅ `57 passed`.
3. Runtime-check:
   - `python -c ... OpenClawClient.health_check()` → `True` (при HTML на `/health`, за счёт CLI-fallback).
   - в `logs/krab_manual_bg.log` зафиксирован новый инфо-лог:
     - `OpenClaw health_check: /health вернул HTML, но CLI probe подтвердил рабочий runtime.`
   - сценарий `stop -> start -> status` (в одном запуске) показывает:
     - fallback PID присутствует;
     - `/api/health` = `UP`.

---

## 📦 Параллельный контур (Antigravity) — R18

Подготовлен единый task-pack для разгрузки квоты Codex и параллельной разработки:

- `/Users/pablito/Antigravity_AGENTS/Краб/docs/ANTIGRAVITY_TASK_PACK_R18_RU.md`

Содержит 3 больших независимых потока:

1. Frontend UX стабилизация health-индикации в Web Panel.

2. Backend fallback-диагностика OpenClaw API (HTML/timeout path).
3. Ops/QA harness для live channel smoke (поиск утечек tool-артефактов).
...
### Дополнительные улучшения (Whisper & Signal)
...
## ✅ v8 Sprint Update 26 (2026-02-23) — Live Channel Smoke Hardening (OpenClaw logs + severity)

### Что реализовано

1. Усилен `scripts/live_channel_smoke.py`:
   - добавлено сканирование логов OpenClaw:
     - `~/.openclaw/logs/gateway.log`
     - `~/.openclaw/logs/gateway.err.log`
   - findings разделены по severity:
     - `error` (блокирует smoke),
     - `warn` (блокирует только при `--strict-warnings`).
   - добавлены новые паттерны:
     - `sanitizer_plugin_config_invalid` (critical),
     - `sanitizer_plugin_untracked_provenance` (warning).
   - добавлены CLI-флаги:
     - `--strict-warnings`
     - `--no-openclaw-logs`

2. Добавлены тесты:
   - `/Users/pablito/Antigravity_AGENTS/Краб/tests/test_live_channel_smoke.py`
   - Проверки:
     - корректная разметка severity,
     - наличие guard-паттернов sanitizer-плагина,
     - корректная обработка отсутствующего лог-файла.

3. Обновлен runbook:
   - `/Users/pablito/Antigravity_AGENTS/Краб/docs/ops_incident_runbook.md`
   - Добавлены новые режимы запуска smoke и перечень OpenClaw логов.

### Верификация

1. `./.venv/bin/python scripts/live_channel_smoke.py --tail-lines 260 --probe-timeout 25` → ✅ `ok=true`
   - `channels_probe: ok`
   - `findings(total/error/warn): 2/0/2` (warning в `~/.openclaw/logs/gateway.log` по raw-токенам, без blocking errors)
2. `pytest -q tests/test_live_channel_smoke.py` → ✅
...
- `tests/test_stream_client_failover.py`
...
) — Voice Gateway Hardening & Ops UX
...
2. Усилен OpenClaw auth
   - **src/main.py**: Точка входа, инициализация `Perceptor`.
...
### Stage 1: Core Voice Handler Integration

- [x] Создать `src/handlers/voice.py` для обработки входящих голосовых.
- [x] Интегрировать Whisper (локально) через `ModelManager`.
- [x] Реализовать базовый цикл: Voice -> STT -> GPT -> TTS -> Voice.

### Stage 2: Multi-engine Support

- [x] Добавить поддержку Ollama/LM Studio для STT/LLM.
- [x] Реализовать переключение движков через `!model scan`.

### Stage 3: Voice Feedback & UI

- [x] Реализовать озвучку ответов через `edge-tts`.
- [x] Починить логику перехвата Thinking-процесса моделей.
...
## ✅ v8 Sprint Update #19 (2026-02-23) — Live Channel Smoke One-Click

### Что реализовано (Update #19)

1. Добавлен one-click smoke-раннер каналов:
   - `/Users/pablito/Antigravity_AGENTS/Краб/scripts/live_channel_smoke.command`
   - Запускает Python smoke и сохраняет окно терминала открытым до `Enter`.

2. Финализирован live smoke скрипт:
   - `/Users/pablito/Antigravity_AGENTS/Краб/scripts/live_channel_smoke.py`
   - Проверяет `openclaw channels status --probe`.
   - Сканирует хвосты логов на запрещенные паттерны утечки (`<|begin_of_box|>`, `I will now call`, `The model has crashed`, `400 No models loaded`).
   - Пишет отчеты в:
...
3. Обновлен runbook:
   - `/Users/pablito/Antigravity_AGENTS/Краб/docs/ops_incident_runbook.md`
   - Добавлен раздел запуска и интерпретации `Live Smoke каналов и sanitizer`.

### Верификация (Update #19)

1. `./.venv/bin/python scripts/live_channel_smoke.py --tail-lines 260 --probe-timeout 25` → ✅
   - `channels_probe: ok`
   - `findings: 0`
   - `ok: true`
2. Проверен артефакт:
   - `/Users/pablito/Antigravity_AGENTS/Краб/artifacts/ops/live_channel_smoke_20260223_175354Z.json`

---

## ✅ v8 Sprint Update #20 (2026-02-23) — LM Studio Idle Guard API-Compat + Runtime Notes

### Что реализовано (Update #20)

1. Усилен idle-guard LM Studio:
   - `/Users/pablito/Antigravity_AGENTS/Краб/scripts/lmstudio_idle_guard.py`
   - Добавлен каскад unload-стратегий:
     - сначала `POST /api/v1/models/unload` с `{"all": true}`,
     - затем fallback по `instance_id`/`instanceReference`,
     - затем fallback по `model`/`model_id`,
     - затем CLI fallback `~/.lmstudio/bin/lms unload --all`.
   - В JSON-отчёт добавлены поля:
     - `unload_mode` (`http`/`cli`),
     - `unload_attempts` (детализация попыток).

2. Добавлены unit-тесты guard:
   - `/Users/pablito/Antigravity_AGENTS/Краб/tests/test_lmstudio_idle_guard.py`
   - Проверки:
     - построение payload-каскада,
     - fallback `all -> instance`,
     - ветки CLI (`lms` отсутствует/успех).

3. Подтверждён runtime-контекст LM Studio (по UI/логам):
   - доступны три совместимых слоя API:
     - `LM Studio API`,
     - `OpenAI-compatible`,
     - `Anthropic-compatible`.

### Верификация (Update #20)

1. `pytest -q tests/test_lmstudio_idle_guard.py tests/test_model_local_health_probe.py` → ✅ `7 passed`.
2. `python3 scripts/lmstudio_idle_guard.py --dry-run --max-idle-minutes 30` → ✅ отчет создан.
3. `python3 scripts/lmstudio_idle_guard.py --max-idle-minutes 30` (без загруженных моделей) → ✅ корректный no-op.

### Важное замечание по live-run из Codex

1. В текущем терминальном рантайме Codex фоновые процессы, запущенные через `nohup ... &`, завершаются после завершения tool-сессии.
2. Из-за этого долгоживущий fallback-процесс `krab_core` из этого канала может выглядеть как «флапающий» (`:8080` UP → DOWN), даже если скрипты старта/логика ядра исправны.
3. Для финальной проверки стабильности `krab_core` нужен запуск daemon-команд из обычного пользовательского shell/macOS-сеанса.

---

## ✅ v8 Sprint Update #26 (2026-02-23) — Krab Core Launchd Fix + Stable HTTP Liveness

### Что реализовано (Update #26)

1. Исправлен запуск `krab_core` через launchd:
   - `/Users/pablito/Antigravity_AGENTS/Краб/krab_core_daemon_start.command`
     - `launchctl enable gui/<uid>/ai.krab.core` теперь выполняется **до** `bootstrap`.
   - `/Users/pablito/Antigravity_AGENTS/Краб/krab_core_daemon_stop.command`
     - stop больше не оставляет сервис в persistent `disabled`-состоянии.

2. В Web API добавлен быстрый liveness endpoint:
   - `/Users/pablito/Antigravity_AGENTS/Краб/src/modules/web_app.py`
   - `GET /api/health/lite` → `{ok:true,status:"up"}` без deep-check зависимостей.

3. Daemon/status/watch переведены на liveness endpoint:
   - `/Users/pablito/Antigravity_AGENTS/Краб/krab_core_daemon_start.command`
   - `/Users/pablito/Antigravity_AGENTS/Краб/krab_core_daemon_status.command`
...
4. Тесты обновлены:
   - `/Users/pablito/Antigravity_AGENTS/Краб/tests/test_web_app.py`
     - добавлен `test_health_lite_endpoint_is_fast_liveness_probe`
     - проверка `health_lite_api` в `/api/links`.

### Верификация (Update #26)

1. `pytest -q tests/test_web_app.py tests/test_krab_core_health_watch.py` → ✅ `44 passed`.
2. `./krab_core_daemon_stop.command && ./krab_core_daemon_start.command && ./krab_core_daemon_status.command`:
   - ✅ launchd `state = running`,
   - ✅ `HTTP health: UP (http://127.0.0.1:8080/api/health/lite)`.
3. `python3 scripts/krab_core_health_watch.py --duration-sec 40 --interval-sec 2`:
   - ✅ `samples=20`, `up/down=20/0`, `flaps=0`, `ok=true`.

### Пояснение по старому флапу `/api/health`

1. `/api/health` — deep ecosystem-check (OpenClaw/local/voice/ear), и может таймаутиться под нагрузкой даже при живом процессе.
2. Для process-liveness используется `/api/health/lite`.
3. `/api/health` оставлен как диагностический deep-check для панели/ops.

---

## ✅ v8 Sprint Update #27 (2026-02-23) — R20 Merge Gate for Parallel Integration

### Что реализовано

1. Добавлен единый интеграционный шлюз для стыковки результатов Antigravity:
   - `/Users/pablito/Antigravity_AGENTS/Краб/scripts/r20_merge_gate.py`
   - `/Users/pablito/Antigravity_AGENTS/Краб/scripts/r20_merge_gate.command`

2. Логика `r20_merge_gate`:
   - required:
     - `pytest -q tests/test_web_app.py tests/test_ecosystem_health.py`
     - compile-check `src/modules/web_app.py` и `src/core/ecosystem_health.py`
     - `GET /api/health/lite`
   - advisory:
     - `GET /api/health`
   - JSON-отчеты:
     - `artifacts/ops/r20_merge_gate_<UTC>.json`
     - `artifacts/ops/r20_merge_gate_latest.json`

### Верификация

1. `python3 scripts/r20_merge_gate.py` → ✅
   - `required_failed=0`
   - `advisory_failed=0`
   - `report=artifacts/ops/r20_merge_gate_20260223_224516Z.json`

---

## ✅ v8 Sprint Update #28 (2026-02-23) — Liveness Probe Timeout Tuning

### Что реализовано

1. Убраны ложные DOWN/fлапы liveness при живом процессе:
   - `/Users/pablito/Antigravity_AGENTS/Краб/scripts/krab_core_health_watch.py`
     - добавлен `--probe-timeout-sec` (default `4`),
     - probe таймаут отвязан от `interval`.
   - `/Users/pablito/Antigravity_AGENTS/Краб/scripts/krab_core_health_watch.command`
     - обновлены default-параметры в подсказке.
   - `/Users/pablito/Antigravity_AGENTS/Краб/krab_core_daemon_start.command`
     - `curl --max-time` для liveness увеличен `2 -> 5`.
   - `/Users/pablito/Antigravity_AGENTS/Краб/krab_core_daemon_status.command`
     - `curl --max-time` для liveness увеличен `2 -> 5`.

### Верификация

1. `python3 scripts/krab_core_health_watch.py --duration-sec 30 --interval-sec 2 --probe-timeout-sec 4`:
   - ✅ `up/down=15/0`, `flaps=0`, `ok=true`.
2. `python3 scripts/r20_merge_gate.py`:
   - ✅ `required_failed=0`, `advisory_failed=0`.
