# Checkpoint для нового чата

- Время: `2026-02-21T18:37:39.230584+01:00`
- Ветка: `codex/queue-forward-reactions-policy`
- HEAD: `75dfe4c`
- Изменённых файлов: `35`

## Что сделано
1. [заполни кратко] Ключевые изменения за текущий этап.
2. [заполни кратко] Какие проверки уже запускались и их итог.

## Что осталось
1. [заполни кратко] Следующий конкретный шаг.
2. [заполни кратко] Риски/блокеры.

## Git status (--short)
```text
M .ralphy/.agent-rules.md
 M .ralphy/templates/Dependencies.md
 M .ralphy/templates/Instruction.md
 M ".ralphy/templates/Project Specs_AGENTS_UPDATED.md"
 M ".ralphy/templates/Project Specs_PRD_Krab_Ultimate.md"
 M .ralphy/templates/prd-template-basic.md
 M .runtime/krab_core.lock
 M HANDOVER.md
 M artifacts/memory/312322764/history.jsonl
 M docs/EXTERNAL_PROMPT_GEMINI3FLASH_BACKEND_R6_RU.md
 M docs/EXTERNAL_PROMPT_GEMINI3FLASH_BACKEND_R9_RU.md
 M docs/EXTERNAL_PROMPT_GEMINI3PRO_FRONTEND_LONG_R12_RU.md
 M docs/EXTERNAL_PROMPT_GEMINI3PRO_FRONTEND_R4_RU.md
 M docs/EXTERNAL_PROMPT_GEMINI3PRO_FRONTEND_R5_RU.md
 M docs/EXTERNAL_PROMPT_GEMINI3PRO_FRONTEND_R6_RU.md
 M docs/EXTERNAL_PROMPT_GEMINI3PRO_FRONTEND_R7_RU.md
 M docs/EXTERNAL_PROMPT_GEMINI3PRO_FRONTEND_R8_RU.md
 M scratch/openclaw_official/data/config/openclaw.json
 M scratch/openclaw_official/src/agents/context-window-guard.ts
 M src/core/openclaw_client.py
 M src/web/index.html
 M src/web/prototypes/nano/index_redesign.html
?? .markdownlint.json
?? REPORTS_R12.md
?? apply_r13.py
?? apply_r13_css.py
?? apply_r13_proper.py
?? docs/R13_FRONTEND_REPORT_RU.md
?? docs/ops_incident_runbook.md
?? scripts/pre_release_smoke.command
?? scripts/test_fallback.py
?? scripts/test_fallback_v2.py
?? scripts/test_openclaw_auth.py
?? tests/test_openclaw_model_autoswitch.py
?? tests/test_signal_ops_guard.py
```

## Последние коммиты
```text
75dfe4c fix(runtime): stabilize autoload, error normalization and test suite
641b817 fix(runtime): harden force_cloud routing and core restart handling
cc07030 docs(prompts): add long R13 backend/frontend antigravity tasks
6e409aa fix(router): robust force_cloud error handling for telegram
7158ffd feat(ops): add one-click pre-release smoke runner with runtime diagnostics
85b230f fix(watchdog): add soft-heal cooldown to prevent repeated RAM unload storms
47bcb16 feat(backend): accept Back12 resilience hardening and web ops lifecycle APIs
fb78adb feat(web): accept Front12 UX hardening and fix duplicate OpenClaw handlers
```

## OpenClaw status
```text
OpenClaw status

Overview
┌─────────────────┬───────────────────────────────────────────────────────────────────────────────────────────────────┐
│ Item            │ Value                                                                                             │
├─────────────────┼───────────────────────────────────────────────────────────────────────────────────────────────────┤
│ Dashboard       │ http://127.0.0.1:18789/                                                                           │
│ OS              │ macos 26.4 (arm64) · node 22.16.0                                                                 │
│ Tailscale       │ off                                                                                               │
│ Channel         │ beta (config)                                                                                     │
│ Update          │ pnpm · npm latest 2026.2.19-2                                                                     │
│ Gateway         │ local · ws://127.0.0.1:18789 (local loopback) · reachable 57ms · auth token · MacBook-Pro-        │
│                 │ Pablito.local (192.168.0.171) app 2026.2.19 macos 26.4                                            │
│ Gateway service │ LaunchAgent installed · loaded · running (pid 777, state active)                                  │
│ Node service    │ LaunchAgent not installed                                                                         │
│ Agents          │ 1 · no bootstraps · sessions 1300 · default main active 6m ago                                    │
│ Memory          │ 2 files · 2 chunks · sources memory · plugin memory-core · vector ready · fts ready · cache on    │
│                 │ (2)                                                                                               │
│ Probes          │ skipped (use --deep)                                                                              │
│ Events          │ none                                                                                              │
│ Heartbeat       │ 30m (main)                                                                                        │
│ Sessions        │ 1300 active · default gemini-2.5-flash (12k ctx) · ~/.openclaw/agents/main/sessions/sessions.json │
└─────────────────┴───────────────────────────────────────────────────────────────────────────────────────────────────┘

Security audit
Summary: 0 critical · 3 warn · 2 info
  WARN Reverse proxy headers are not trusted
    gateway.bind is loopback and gateway.trustedProxies is empty. If you expose the Control UI through a reverse proxy, configure trusted proxies so local-client c…
    Fix: Set gateway.trustedProxies to your proxy IPs or keep the Control UI local-only.
  WARN Gateway token looks short
    gateway auth token is 15 chars; prefer a long random token.
  WARN Some configured models are below recommended tiers
    Smaller/older models are generally more susceptible to prompt injection and tool misuse. - openai/gpt-4o-mini (Below GPT-5 family) @ agents.defaults.model.fall…
    Fix: Use the latest, top-tier model for any bot with tools or untrusted inboxes. Avoid Haiku tiers; prefer GPT-5+ and Claude 4.5+.
Full report: openclaw security audit
Deep probe: openclaw security audit --deep

Channels
┌─────────────┬─────────┬────────┬────────────────────────────────────────────────────────────────────────────────────┐
│ Channel     │ Enabled │ State  │ Detail                                                                             │
├─────────────┼─────────┼────────┼────────────────────────────────────────────────────────────────────────────────────┤
│ Telegram    │ ON      │ OK     │ token config (8234…ui3w · len 46) · accounts 1/1                                   │
│ WhatsApp    │ ON      │ OK     │ linked · +34603834299 · auth 3m ago · accounts 1                                   │
│ Discord     │ ON      │ OK     │ token config (MTQ3…ava4 · len 72) · accounts 1/1                                   │
│ Slack       │ ON      │ OK     │ tokens ok (bot config, app config) (bot xoxb…Jjoi · len 55, app xapp…9144 · len    │
│             │         │        │ 98) · accounts 1/1                                                                 │
│ Signal      │ ON      │ OK     │ configured                                                                         │
│ iMessage    │ ON      │ OK     │ configured                                                                         │
│ BlueBubbles │ ON      │ WARN   │ Not configured (missing serverUrl or password). · gateway: Not configured          │
│             │         │        │ (missing serverUrl or password).                                                   │
└─────────────┴─────────┴────────┴────────────────────────────────────────────────────────────────────────────────────┘

Sessions
┌───────────────────────────────────────────────────────────┬────────┬─────────┬──────────────────┬───────────────────┐
│ Key                                                       │ Kind   │ Age     │ Model            │ Tokens            │
├───────────────────────────────────────────────────────────┼────────┼─────────┼──────────────────┼───────────────────┤
│ agent:main:openai:00b0d7cd-032e…                          │ direct │ 6m ago  │ local            │ 16k/12k (133%)    │
│ agent:main:telegram:direct:3123…                          │ direct │ 2h ago  │ local            │ 14k/12k (119%)    │
│ agent:main:main                                           │ direct │ 2h ago  │ local            │ unknown/12k (?%)  │
│ agent:main:openai:a5b68885-0023…                          │ direct │ 2h ago  │ gemini-2.5-flash │ unknown/200k (?%) │
│ agent:main:openai:33e11d57-720c…                          │ direct │ 2h ago  │ local            │ unknown/12k (?%)  │
│ agent:main:openai:3531701f-fa55…                          │ direct │ 2h ago  │ local            │ unknown/12k (?%)  │
│ agent:main:openai:9456108c-90e5…                          │ direct │ 2h ago  │ local            │ unknown/12k (?%)  │
│ agent:main:openai:62d84305-a413…                          │ direct │ 2h ago  │ local            │ unknown/12k (?%)  │
│ agent:main:openai:0febcd79-930d…                          │ direct │ 2h ago  │ local            │ unknown/12k (?%)  │
│ agent:main:openai:1680f035-b2a8…                          │ direct │ 2h ago  │ local            │ unknown/12k (?%)  │
└───────────────────────────────────────────────────────────┴────────┴─────────┴──────────────────┴───────────────────┘

FAQ: https://docs.openclaw.ai/faq
Troubleshooting: https://docs.openclaw.ai/troubleshooting

Next steps:
  Need to share?      openclaw status --all
  Need to debug live? openclaw logs --follow
  Need to test channels? openclaw status --deep
```

## Workstream overlap check
```text
🧭 Workstream Overlap Check
- antigravity patterns: 15
- codex patterns: 13
- gemini_design patterns: 3
- nanobanana_ui patterns: 3
- Stream pairs: 6
- Repo overlaps: 0
- Changed-file overlaps: 0

✅ Конфликтов в измененных файлах не обнаружено.
```

## Paste в новый чат (шаблон)
```text
[CHECKPOINT]
branch=codex/queue-forward-reactions-policy
head=75dfe4c
changed_files=35
scope=[что именно делаем дальше]
done=[что уже готово и проверено]
next=[ближайший шаг]
risks=[кратко]
```
