...
Ветка реализации: `codex/reserve-roundtrip-e2e`
...
1. На `USER2` держать truthful recovery и не смешивать API-key / bypass-контуры со штатным OAuth snapshot.
2. Добить всё, что не привязано к домашней директории `pablito`, включая shared-sync и документацию.
3. На `pablito` пройти account-local relogin через owner panel и только потом переподтвердить merge-ready тем же release-gate циклом.
